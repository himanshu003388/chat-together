...
export default function RoomChat({ roomId, roomName, currentUser }: RoomChatProps) {
      ...
      .on('postgres_changes', {
        ...
        try {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            let profile = profilesRef.current.get(newMsg.sender_id);
            if (!profile) {
              ...
              if (profile) {
                profilesRef.current.set(newMsg.sender_id, profile);
                usernameToIdRef.current.set(profile.username.toLowerCase(), newMsg.sender_id);
              }
            }
            ...
            if (newMsg.reply_to) {
              const parent = messagesRef.current.find(m => m.id === newMsg.reply_to);
              if (parent) {
                newMsg.reply_message = { content: parent.content, profiles: parent.profiles };
              } else {
                const { data: parentDb } = await supabase
                  ...
                  .maybeSingle();
                newMsg.reply_message = parentDb as any;
              }
            }
    ...
    try {
      ...
      if (data) {
        ...
        messagesWithReplies.forEach((msg: any) => {
          if (msg.profiles) {
            profilesRef.current.set(msg.sender_id, msg.profiles);
            usernameToIdRef.current.set(msg.profiles.username.toLowerCase(), msg.sender_id);
          }
        });
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const sent = await chatService.sendMessage(currentUser.id, content, roomId, null, replyId);
      if (sent) {
        setMessages(prev => {
          if (prev.some(m => m.id === sent.id)) return prev;
          const filtered = prev.filter(m => !(m.id.startsWith('temp-') && m.sender_id === currentUser.id));
                  ...
                  <div className={cn("flex flex-col", isMe ? 'items-end' : 'items-start', "max-w-[90%] sm:max-w-[70%]")}>
                    ...
                    <div className="relative group/bubble">
                      ...
                      <div className={cn(
                        ...
                        <button onClick={(e) => { e.stopPropagation(); setReplyingTo(msg); setSelectedMessageId(null); }} className="p-2.5 hover:bg-white/5 rounded-xl text-white/40 hover:text-white transition-colors" title="Reply"><ReplyIcon className="w-5 h-5" /></button>
                        <button onClick={(e) => { e.stopPropagation(); setShowEmojiPicker(showEmojiPicker === msg.id ? null : msg.id); }} className="p-2.5 hover:bg-white/5 rounded-xl text-white/40 hover:text-white transition-colors" title="React"><Smile className="w-5 h-5" /></button>
              ...
              <div className="p-4 glass-card bg-accent-purple/5 border-accent-purple/20 flex items-start justify-between rounded-2xl border-l-4 border-l-accent-purple">
                <div className="flex gap-4 overflow-hidden">
                  ...
                  <div className="flex flex-col overflow-hidden">
                    ...
                    <p className="text-sm text-white/60 truncate italic leading-relaxed">"{replyingTo.content}"</p>
                  </div>
                </div>
