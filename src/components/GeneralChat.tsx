...
export default function GeneralChat({ currentUser }: GeneralChatProps) {
  ...
  const usernameToIdRef = useRef<Map<string, string>>(new Map());
  const lastRealtimeEvent = useRef<number>(Date.now());
  const pollingInterval = useRef<ReturnType<typeof setInterval> | null>(null);
  
  const chatService = useMemo(() => new ChatService(supabase), []);

  // Polling fallback: only runs if no realtime event arrived in 5s
  useEffect(() => {
    const POLL_MS = 3000;
    const STALE_MS = 5000;
    pollingInterval.current = setInterval(() => {
      if (Date.now() - lastRealtimeEvent.current > STALE_MS) {
        fetchMessages();
      }
    }, POLL_MS);
    return () => {
    ...
    const messagesChannel = supabase
      ...
      .on('postgres_changes', {
        ...
        try {
          ...
          if (msg.chat_id || msg.sender_id === currentUser.id) return;
          let profile = profilesRef.current.get(msg.sender_id);
          if (!profile) {
            const { data } = await supabase
              .from('profiles').select('id, username, avatar_url')
              .eq('id', msg.sender_id).maybeSingle();
            profile = data || undefined;
            if (profile) profilesRef.current.set(msg.sender_id, profile);
          }
      ...
      .subscribe((status) => {
        ...
        if (status === 'SUBSCRIBED') lastRealtimeEvent.current = Date.now();
      });
    ...
    const extraChannel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'reactions' }, (payload) => {
        if (payload.eventType === 'INSERT') {
          ...
          setMessages(prev => prev.map(m =>
            m.id === old.message_id
              ? { ...m, reactions: (m.reactions || []).filter(r => r.id !== old.id) }
              : m
          ));
        }
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'pinned_messages' }, (payload) => {
        if (payload.eventType === 'INSERT') {
          setPinnedMessages(prev => [...prev, (payload.new as any).message_id]);
        } else if (payload.eventType === 'DELETE') {
          setPinnedMessages(prev => prev.filter(id => id !== (payload.old as any).message_id));
        }
      })
      .subscribe();
  ...
  const fetchMessages = async () => {
    try {
      ...
      if (data) {
        const replyIds = (data as any[])
          .filter(m => m.reply_to && !data.find((d: any) => d.id === m.reply_to))
          .map(m => m.reply_to);
        ...
        if (replyIds.length > 0) {
          ...
          const { data: parentMessages } = await supabase
            ...
            .in('id', uniqueIds);
          if (parentMessages) {
            parentMessages.forEach(m => replyMap[m.id] = m);
          }
        }

        const messagesWithReplies = (data as any[]).map(msg => {
          if (msg.reply_to) {
            msg.reply_message = data.find((m: any) => m.id === msg.reply_to) || replyMap[msg.reply_to] || null;
          }
          return msg;
        });
  ...
  const fetchAllProfiles = async () => {
    ...
    if (data) {
      setAllProfiles(data);
      data.forEach(p => {
        profilesRef.current.set(p.id, { id: p.id, username: p.username, avatar_url: p.avatar_url });
        usernameToIdRef.current.set(p.username.toLowerCase(), p.id);
      });
    }
  };
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const sent = await chatService.sendMessage(currentUser.id, newMessage.trim() || null, null, null, replyTo?.id || null, fileInfo);
      ...
      if (sent) {
        ...
        let match;
        while ((match = mentionRegex.exec(newMessage.trim())) !== null) {
          const username = match[1].toLowerCase();
          const userId = usernameToIdRef.current.get(username);
        ...
        if (mentions.length > 0) {
          fetch('/api/notifications/mention', {
            ...
            body: JSON.stringify({
              ...
              mentionedByUsername: currentUser.username || currentUser.email,
                ...
                <div className={cn("flex flex-col", isMe ? 'items-end' : 'items-start', "max-w-[90%] sm:max-w-[70%]")}>
                  ...
                  <div className="relative group/bubble">
                    ...
                    <div className={cn(
                      ...
                      <button onClick={(e) => { e.stopPropagation(); setReplyTo(msg); setSelectedMessageId(null); }} className="p-2.5 hover:bg-white/5 rounded-xl text-white/40 hover:text-accent-cyan transition-colors" title="Reply"><MessageCircle className="w-5 h-5" /></button>
                      <button onClick={(e) => { e.stopPropagation(); setShowEmojiPicker(showEmojiPicker === msg.id ? null : msg.id); }} className="p-2.5 hover:bg-white/5 rounded-xl text-white/40 hover:text-accent-cyan transition-colors" title="React"><Smile className="w-5 h-5" /></button>
                      <button onClick={(e) => { e.stopPropagation(); togglePin(msg.id); setSelectedMessageId(null); }} className={cn("p-2.5 rounded-xl transition-colors", isPinned ? 'text-accent-cyan bg-accent-cyan/10' : 'text-white/40 hover:text-white hover:bg-white/5')} title="Pin"><Pin className="w-5 h-5" /></button>
                      {isMe && (
                        <>
                          <button onClick={(e) => { e.stopPropagation(); setEditingId(msg.id); setEditContent(msg.content || ''); setSelectedMessageId(null); }} className="p-2.5 hover:bg-white/5 rounded-xl text-white/40 hover:text-accent-cyan transition-colors" title="Edit"><Edit3 className="w-5 h-5" /></button>
                          <button onClick={(e) => { e.stopPropagation(); deleteMessage(msg.id); setSelectedMessageId(null); }} className="p-2.5 hover:bg-accent-pink/5 rounded-xl text-white/40 hover:text-accent-pink transition-colors" title="Delete"><Trash2 className="w-5 h-5" /></button>
                        </>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowUserProfile(null)}>
          ...
          >
            ...
            <div className="text-center">
              ...
              <button
                onClick={() => window.location.href = `/chat/${showUserProfile.id}`}
