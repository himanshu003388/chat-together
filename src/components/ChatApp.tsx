...
export default function ChatApp({
  ...
  const activeUserIdRef = useRef<string | null>(initialActiveUserId || null);
  ...
  useEffect(() => {
    ...
    pollingInterval.current = setInterval(() => {
      if (Date.now() - lastRealtimeEvent.current > STALE_MS && activeUserIdRef.current) {
        fetchMessages();
      }
    }, POLL_MS);
    return () => {
  ...
  useEffect(() => {
    const messagesChannel = supabase
      ...
      .on('postgres_changes', {
        ...
        if (uid && (msg.sender_id === uid || msg.receiver_id === uid)) {
          setMessages(prev => {
    ...
    const profilesChannel = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'profiles' }, (payload) => {
        const p = payload.new as Profile;
        setProfiles(prev => prev.map(profile => profile.id === p.id ? { ...profile, ...p } : profile));
      })
      .subscribe();
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const sent = await chatService.sendMessage(currentUser.id, tempMsg.content || null, null, activeUserId, null, fileInfo);
      ...
      if (sent) {
        ...
        fetch('/api/notifications/dm', {
          ...
          body: JSON.stringify({
            ...
            messageContent: tempMsg.content,
          }),
        }).catch(() => {});
      }
